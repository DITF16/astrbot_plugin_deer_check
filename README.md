...
-   **群组白名单**: 设置允许使用插件的群号列表，留空表示所有群都可使用
-   **用户黑名单**: 设置禁止使用插件的用户列表，黑名单中的用户消息将被忽略
-   **每天开始时间**: 设置每天的开始时间，格式为HH:MM，例如设置为02:00表示凌晨2点前的操作算作前一天
-   **自动删除上月数据**: 启用后每月初自动删除上月数据，禁用则保留历史数据可查看往月记录
-   **每日最多计入次数**: 设置每天最多计入的打卡次数，超过此数量将不再记录，设置为0表示无限制
-   **每月最多计入次数**: 设置每月最多计入的打卡次数，超过此数量将不再记录，设置为0表示无限制
-   **启用女性适用日历**: 启用专门的女性适用日历功能（此功能暂未实现）
-   **排行展示用户数量**: 设置排行榜显示的用户数量，默认为10
